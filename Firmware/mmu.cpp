...
void mmu_loop(void)
{
  ...
  if ((mmu_state == 1) && mmu_cmd) {
    if ((mmu_cmd >= MMU_CMD_T0) && (mmu_cmd <= MMU_CMD_T4))
    {
      ...
      } else uart2_txPayload(tempTxCMD);
    ...
    mmu_cmd = 0;
  } else if (((mmu_last_response + 300) < millis()) && !mmuFSensorLoading && (mmu_state == 1)) {
    //request every 500ms
    uart2_txPayload("P0-");
    mmu_state = 2;
...
{
  if (shutdown && ((tmc2130_current_h[3] != 0) && (tmc2130_current_r[3] != 0))) {
      mmuE0BackupCurrents[0] = tmc2130_current_h[3];
      mmuE0BackupCurrents[1] = tmc2130_current_r[3];
      tmc2130_set_current_h(3, 0);
      tmc2130_set_current_r(3, 0);
  } else if (!shutdown) {
      tmc2130_set_current_h(3, mmuE0BackupCurrents[0]);
      tmc2130_set_current_r(3, mmuE0BackupCurrents[1]);
  }
}
